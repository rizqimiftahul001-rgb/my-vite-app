...
import ProviderManagementHeader from "./userLimitHeader";
...
function userLimitManagement() {
  ...
  const { casinoUser } = queryString.parse(search);

  const [resultWins, setResultWins] = useState([]);

  const [userInfo, setUserInfo] = useState();

  const casinoUserInfo = () => {
    APIService({
      url: `${process.env.REACT_APP_R_SITE_API}/user/get-casino-user-detail?user_id=${casinoUser}`,
      method: "GET",
    }).then((data) => {
  ...
  const getAllWinList = () => {
    APIService({
      url: `${process.env.REACT_APP_R_SITE_API}/betlimit/get-user-bet-lists/${casinoUser}`,
      method: "GET",
    }).then((data) => {
  ...
  // console.log("selected ValueGameSpec :", selectedValueGameSpec);
  // Specific Game
  ...
  const saveAndUpdate = () => {
    ...
    if (flag === false) {
      ...
      if (
        ...
        if (existingLimitByVendor && existingLimitByVendor !== undefined ? existingLimitByVendor.vendor === selectedValueVendor : existingLimitByVendor) {
          error_f_alre_exist = true;
          return dispatch(
            showMessage({
              variant: "error",
      ...
      if (error_f_alre_exist == false) {
        _saving(true);
        APIService({
          url: `${process.env.REACT_APP_R_SITE_API}/betlimit/add-user-max`,
          method: "POST",
          data: payload,
        })
          .then((data) => {
  ...
  const actionWinting = ({ unique_id, type, action }) => {
    ...
    APIService({
      url: `${process.env.REACT_APP_R_SITE_API}/betlimit/action-user`,
      method: "PUT",
      data: payload,
    })
      .then((data) => {
            ...
            >
              <div className="" style={{ width: "100%" }}>
                ...
                <div>
                  ...
                  >
                    <b>{selectedLang.the_maximum_bet_limit_is_applied_following_order}</b>
                  </Typography>
                </div>
                  ...
                  <div className="modal_inner" style={{ marginTop: "10px" }}>
                    <div className="table-responsive">
                      <table className="table">
                        <thead>
                          <tr>
                            <th>{selectedLang.classification_of_settings}</th>
                          ...
                          {showGameSpecList &&
                            gameLimitArr?.map((data, key) => (
                              <tr key={key}>
                                <td>
                                  {" "}
                                  {selectedLang.game_specific_settings}
                                </td>
                                <td>{data?.title}</td>
                                <td>
                                  <span className="text-buttons">
...
export default userLimitManagement;
