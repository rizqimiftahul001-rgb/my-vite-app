...
import UserRevenueStatisticsHeader from "./providerStatisticsByGameHeader";
...
function statisticsByGameApp() {
  ...
  const columns = [
    ...
    { id: "provider_name", label: `${selectedLang.provider}`, minWidth: 50 },
  ...
  const getStatData = (date) => {
    ...
    APIService({
      url: `${process.env.REACT_APP_R_SITE_API
        }/revenue/provider-statistics-by-game?user_id=${!agent_id ? user_id : agent_id
        }&&start_date=${startDate}&end_date=${endDate}&pageNumber=${1}&limit=${5000}&req_agent_data=${agent_id ? agent_id : ""
        }&filterAgent=&agent_filter=${filterAgent}`,
      method: "GET",
    })
      .then((res) => {
          ...
          {sortedAndMappedstatData
            ...
            .map((stat, index) => (
              <StyledTableRow hover role="checkbox" tabIndex={-1} key={index}>
                ...
                >
                  {Number(stat.refund_total_sum)?.toLocaleString()}
                </TableCell>
                <TableCell
                  sx={{
                    textAlign: "center",
                ...
                >
                  {Number(stat.cancel_total_sum)?.toLocaleString()}
                </TableCell>
                {/* <TableCell
                  style={{
                    textAlign: "center",
                ...
                <TableCell
                  style={{
                    ...
                    fontWeight: "bold",
                    color:
                      Number(stat.sub_profit_or_loss) < 0
                        ? "red"
                        : Number(stat.sub_profit_or_loss) > 0
                        ? "#35cdd9"
                        : "white",
            ...
            >
              <div className="flex justify-start justify-items-center bg-gray p-10 list_title w-100">
                <span className="list-title">
                  {selectedLang.statisticsProviderByGame}
                </span>
              </div>
...
export default statisticsByGameApp;
