...
function GameStatusManagementApp() {
  ...
  const fetchGameList = () => {
    ...
    APIService({
      url: `${process.env.REACT_APP_R_SITE_API}/user/games-list-status-mt?limit=${rowsPerPage}&page=${page}&title=${title}&vendor=${vendor}&id=${id}&value=${value}&gameId=${gaameIdSearch}&provider_game_id=${providerGameIdsearch}`,
      method: "GET",
    })
      .then((res) => {
  ...
  const handleChangePlayabelStatus= (real_id,id, provider_game_id, is_playable)=>{

    APIService({
      url: `${process.env.REACT_APP_R_SITE_API}/user/update-playable-game-status`,
      method: "PUT",
      data: {
        real_id,
        id,
...
export default GameStatusManagementApp;
